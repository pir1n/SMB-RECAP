...
READER="streaming"
BENCH_MODULE="smb_recap.benchmark_parsepcap.cli"
TOOL_MODE="both"
...
Options:
  ...
  --bench-module MODULE     Benchmark module. Default: smb_recap.benchmark_parsepcap.cli.
...
run_one_repeat() {
  ...
  echo "    [SMB-RECAP] parse run $run_id"
  smb_start="$(now_ns)"

  if "$PYTHON_BIN" -m smb_recap parse-pcap \
      "$pcap" \
      "$out_json" \
