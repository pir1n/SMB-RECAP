...
from smbmount.pcapfs.output.snapshot_export import build_snapshot
from smbmount.shared.parser.utils import *
from smbmount.shared.parser.smb2_constants import *
from smbmount.shared.parser.smb2_extractors import *
from smbmount.shared.parser.tcp_reassembler import (
...
def _find_smb2_payload_layer(payload_pkt):
    ...
    if SMB2_Query_Directory_Response is not None:
        payload_classes.append(SMB2_Query_Directory_Response)
...
def enrich_with_request_mapping(packets):
    ...
    # FileId is scoped by the SMB session/tree, not by a TCP transport.
    # SMB 3 Multichannel may use a handle opened on one TCP connection from
    # another connection in the same session.
    file_context = {}  # (client_ip, server_ip, session_id, tree_id, file_id_key) -> context
    ...
    def build_file_context_key(pkt, file_id, forced_client_flow=None):
        ...
        flow = forced_client_flow or client_flow_key(pkt)
        return (
            flow[0],
            flow[1],
    ...
    query_response_fields = [
        "smb2_query_directory_response_data",
        "smb2_query_entry_count",
        "smb2_query_returned_names",
        "smb2_query_end_of_search",
        "smb2_query_response_status",
        "smb2_query_total_bytes",
    ]
    ...
    for pkt in packets:
        ...
        if is_response is True:
            ...
            if req.get("smb2_command_name") == "QUERY_DIRECTORY":
                data = pkt.get("smb2_query_directory_response_data")
                file_class = req.get("smb2_file_info_class_raw")
                names = parse_query_directory_entries(data, file_class)
                status = pkt.get("smb2_status")
                status_text = str(status or "")
                pkt["smb2_query_returned_names"] = names
                pkt["smb2_query_entry_count"] = len(names)
                pkt["smb2_query_total_bytes"] = len(data or b"")
                pkt["smb2_query_response_status"] = status
                pkt["smb2_query_end_of_search"] = (
                    True if (
                        "NO_MORE_FILES" in status_text.upper()
                        or status_text in {"2147483654", "0x80000006"}
                    )
                    else False if data
                    else None
                )
                for field in query_response_fields:
                    copy_field(pkt, req, field)
...
def enrich_with_file_metadata_mapping(packets: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for pkt in packets:
        ...
        if cmd == "CREATE" and is_response is True:
            create_metadata_by_file_id[metadata_key(pkt, file_id)] = {
                field: pkt.get(field)
        ...
        if cmd in ("READ", "WRITE", "CLOSE", "SET_INFO", "QUERY_INFO"):
            metadata = create_metadata_by_file_id.get(metadata_key(pkt, file_id))
            if not metadata:
                continue
